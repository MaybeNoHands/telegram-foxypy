...
loops = int(input('Сколько чатов создать: '))
...
invitee_user = client.get_input_entity(invite_user)


def create_chats(i):
    random_symbols = ''.join(random.choices(string.ascii_letters + string.digits, k=5))
    chat_title = f'чат {i} {random_symbols}'
    ...
    client(InviteToChannelRequest(
        channel=InputChannel(chat_id, response.chats[0].access_hash),
        users=[invitee_user]
    ))
    ...
    with open(f'{target_user}_chats.txt', 'a', encoding='utf8') as f:
